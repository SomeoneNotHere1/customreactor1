...
let placeOptions=[
	...
	{//7
		...
		x:10,
		//my lazy has supassed enough boundaries to allow logic to happen, allowing eaiser explanation and execution, of my 
		y:((50+75)+(75+75))*2,//MY POWER GROWS
		draw(){
       	var c=document.getElementById("myCanvas");
        	var ctx=c.getContext("2d");
        	var imagee = new Image();
...
function addFloor(xx,yy,io,tex){
	floor.push(
		{
			x:xx,
			y:yy,
		...
		if(gameInfo.placing){
			...
			if(is_mouse_in_area()){
				if(gameInfo.placing != 'sellTower.png' && gameInfo.money>gameInfo.price){
					...
					if(convs.includes(tex)){
						gameInfo.money+=2
					}
					if(tex=='upgraderblue.png'){
						gameInfo.money+=10
					}
					if(tex=='upgraderred.png'){
						gameInfo.money+=12
					}
					if(tex=='upgraderpurple.png'){
						gameInfo.money+=14
					}
					if(tex=='upgraderblack.png'){
						gameInfo.money+=16
					}
					if(tex=='seller.png'){
						gameInfo.money+=2
					}
					if(tex=='generator.png'){
						gameInfo.money+=10
					}
